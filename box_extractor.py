...
def find_boxes(image: np.ndarray) -> np.ndarray:
    ...
    image = cv.bilateralFilter(image, 10, 50, 50)  # removes noise but keeps edges sharp

    image = cv.cvtColor(image, cv.COLOR_BGR2GRAY)

    N = 32
    image = (np.round(image * (N / 255), 0) * (255 / N)).astype(np.uint8)  # solarize image to N colors

    # apply threshold to image to convert it into black & white
    _, mask = cv.threshold(image, 160, 255, cv.THRESH_BINARY_INV)
