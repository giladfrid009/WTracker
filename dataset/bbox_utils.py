...
class BoxConverter:
    ...
    @staticmethod
    def _unpack(bbox: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        return np.split(bbox.astype(int), bbox.shape[-1], axis=-1)

    @staticmethod
    def _pack(c1: np.ndarray, c2: np.ndarray, c3: np.ndarray, c4: np.ndarray) -> np.ndarray:
    ...
    def change_format(bbox: np.ndarray, src_format: BoxFormat, dst_format: BoxFormat) -> np.ndarray:
        ...
        if (src_format, dst_format) == (BoxFormat.XYXY, BoxFormat.XYWH):
            x1, y1, x2, y2 = BoxConverter._unpack(bbox)
            w = x2 - x1
            h = y2 - y1
            return BoxConverter._pack(x1, y1, w, h)
            ...
            xm = x1 + w // 2
            ym = y1 + h // 2
            return BoxConverter._pack(xm, ym, w, h)

        elif (src_format, dst_format) == (BoxFormat.XYWH, BoxFormat.XYXY):
            x1, y1, w, h = BoxConverter._unpack(bbox)
            x2 = x1 + w
            y2 = y1 + h
            return BoxConverter._pack(x1, y1, x2, y2)
            ...
            xm, ym, w, h = BoxConverter._unpack(bbox)
            x1 = xm - w // 2
            y1 = ym - h // 2
            x2 = x1 + w
            y2 = y1 + h
